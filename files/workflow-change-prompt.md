...
Your task is to semantically analyze the current tables and the change description, then produce updated versions of both tables that reflect the requested changes — while respecting all schema rules and lock constraints. Whem you adding element remember to add all required connections.

Please check if and what kind of lock is on each element, in case you are not able to bypass the lock write list of validations what was not reflected in the tables. You generate same type of output - two files
...
| lock_keyword | lock_noterasable, lock_noteditable | For user_action_process only | lock_noterasable = lock_noterasable = cannot delete but may edit it or subprocesses within anyhow; lock_noteditable = fully locked |
